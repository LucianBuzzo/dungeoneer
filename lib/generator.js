...
const Dungeon = function Dungeon() {
  ...
  const generate = (stage) => {
    ...
    for (var y = 0; y < stage.height; y += 2) {
      for (var x = 0; x < stage.width; x += 2) {
    ...
    while (cells.length && count < 500) {
      ...
      if (unmadeCells.length) {
        ...
        if (unmadeCells.indexOf(lastDir) > -1 && _.random(1, 100) > windingPercent) {
          ...
          dir = unmadeCells[rand];
          dir2 = unmadeCells2[rand];
        }

        let [dirX, dirY] = dir.split(':').map(Number);
        let [dirX2, dirY2] = dir2.split(':').map(Number);

        _carve(dirX, dirY);
        _carve(dirX2, dirY2);

        cells.push({ x: dirX2, y: dirY2 });
        lastDir = dir;
        lastDir2 = dir2;
      } else {
  ...
  const _addRooms = () => {
    for (var i = 0; i < numRoomTries; i++) {
      ...
      var x = _.random(1, stage.width - width - 1);
      var y = _.random(1, stage.height - height - 1);
