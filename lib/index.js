...
const Dungeon = function Dungeon () {
  ...
  const _addRooms = () => {
    ...
    for (let i = 0; i < numRoomTries && _rooms.length < maxNumberOfRooms; i++) {
      ...
      const room = new Room(x, y, width, height, _currentRegion + 1)
  ...
  const _connectRegions = () => {
    ...
    const roomCorners = _getRoomCorners()
    _tiles.forEach(row => {
      row.forEach(tile => {
        if (tile.type === 'floor') {
          return
        ...
        if (tile.getTileNESW().find(tile => roomCorners[tile.x].includes(tile.y))) {
          return
        }

        const tileRegions = _.unique(
          tile.getTileNESW().map(x => x.region)
    ...
    _.each(regionConnections, (connections, connKey) => {
      const index = randBetween(0, connections.length - 1)
      _addDoor(connections[index], connKey)
      connections.splice(index, 1)

      // Occasional open up additional connections
      connections.forEach(conn => {
        if (_oneIn(extraConnectorChance)) {
          _addDoor(conn, connKey)
        }
      })
    })
  }

  const _addDoor = (tile, connKey) => {
    const regions = connKey.split('-')
    regions.forEach(region => {
      const room = _rooms.find(room => room.region === Number(region))
      if (room != null) {
        room.doors.push(tile)
        tile.rooms.push(room)
      }
    })
    tile.type = 'door'
  }
  ...
  const _lootifyDeadEnds = () => {
    ...
    _tiles.forEach((row) => {
      row.forEach((tile) => {
        ...
        if (
          tile.getTileNESW().filter(t => t.type !== 'wall').length <= 1 &&
          ...
          for (let i = 0; i < minBigLootLength; i++) {
            const adjacentTiles = currentTile.getTileNESW()
  ...
  const _getRoomCorners = function () {
    const borders = Array.from({ length: _tiles.length + 1 }, () => [])
